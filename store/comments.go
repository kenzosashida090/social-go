...
func (s *CommentStorage) Create(ctx context.Context, comment *Comment) error {
	...
	`
	err := s.db.QueryRowContext(ctx, query, comment.PostID, comment.UserID, comment.Content).Scan(&comment.ID, &comment.PostID, &comment.UserID, &comment.Content)
	if err != nil {
		return err
	}
	return nil
}
func (s *CommentStorage) GetCommentsByUserId(ctx context.Context, postID int64) ([]Comment, error) {
	query := `
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Println("----------------COMMMMMMMMMMMMMMEEEEEEEEEEEEEEENTS", &rows)
			return nil, err
		}
